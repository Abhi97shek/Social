...
else
{
		...
		if(!$result1)
	{
		...
		$result=mysqli_query($connection,$query);
		$row=mysqli_fetch_row($result);							
		$getquery="SELECT sender_id FROM friendrequest WHERE receiver_id='$row[0]' AND request='1'";	
		$resultfriend=mysqli_query($connection,$getquery);
		$request=array();
		while($fetch=mysqli_fetch_row($resultfriend))
		{
		$request[]=$fetch[0];
		}
		$users=implode(',',$request);
$queryfriend="SELECT Firstname,Lastname,Profileimage,userid From Users_meta WHERE userid IN($users)";
    ...
    <main class="mainsection">
    	<div class="container row">
            ...
            <div class="col-lg-7">
                  ...
                  echo	"<button type='button' value='AddFriend' data-accept=".$useridarray[$i]." class='btn btn-primary acceptbutton'>Accept</button>";
				  echo	"<button type='button' value='AddFriend' data-delete=".$useridarray[$i]." class='btn btn-primary deletebutton'>Delete</button>";
